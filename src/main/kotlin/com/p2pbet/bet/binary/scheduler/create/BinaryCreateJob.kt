package com.p2pbet.bet.binary.scheduler.create
...
class BinaryCreateJob(
    ...
    override fun execute(context: JobExecutionContext) {
        val jobData = runCatching { context.getDataObject(BinaryCreateJobData::class.java) }
            .getOrElse {
    ...
    fun execute(jobData: BinaryCreateJobData, context: JobExecutionContext) {
        runCatching {
            logger.info { "Start binary creation with data: $jobData" }
            if (jobData.executionId == null) {
                initNewCreate(jobData, context)
            } else {
                checkExecutionStatus(jobData, context)
            }
            logger.info { "End of binary creation with data: $jobData" }
        }.onFailure {
    ...
    private fun checkExecutionStatus(jobData: BinaryCreateJobData, context: JobExecutionContext) =
        when (biExecutionService.checkStatus(jobData.executionId!!, jobData.executionType)) {
            ...
            ExecutionStatus.FAILED -> {
                logger.info { "Failed during binary bet creation with data: $jobData" }
                context.deleteCurrentJob()
            }
        }

    private fun initNewCreate(jobData: BinaryCreateJobData, context: JobExecutionContext) {
        logger.info { "Init new create call for binary" }

        val executionId = biExecutionService.asyncWriteCall(
            executionAction = ExecutionAction.CREATE_BINARY_BET,
            betId = null
        ) {
            binaryActionService.callCreateBinary(
