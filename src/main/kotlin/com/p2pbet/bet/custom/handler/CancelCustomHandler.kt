...
@Component
class CancelCustomHandler(
    val customMatchingService: CustomMatchingService,
) : CustomAbstractHandler(
    logType = LogEnumMapper.CUSTOM_CANCELED
) {
    private val logger: KLogger = KotlinLogging.logger { }

    override fun handle(action: AbstractLog, executionType: BetExecutionType): Unit =
        with(action as CustomBetCanceledLog) {
            logger.info { "Start CUSTOM_CANCELED handler" }
            customMatchingService.onCustomCancel(this, executionType)
        }
}